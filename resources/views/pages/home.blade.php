<x-layouts.site title="Home — The Deborah Bonat Foundation">
    ...
    {{-- Hero --}}
    <section class="relative min-h-[85vh] flex items-center justify-center bg-brand-dark overflow-hidden">
        {{-- Placeholder image overlay --}}
        <div class="absolute inset-0 bg-gradient-to-br from-brand-dark via-brand-purple/70 to-brand-dark"></div>
        <div class="absolute top-4 right-4 bg-white/10 text-white/30 text-xs px-3 py-1 rounded-full font-mono hidden sm:block">[Hero photo — replace me]</div>

        <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center py-16">
            <p class="text-brand-gold text-xs font-semibold uppercase tracking-widest mb-4">The Deborah Bonat Foundation</p>
            ...
            <p class="mt-6 text-white/70 text-lg max-w-xl mx-auto leading-relaxed">
                Serving widows, orphans, the elderly, missionaries, and new converts across Nigeria
                — in honour of Mama Deborah Ishaku Bonat.
            </p>
            <div class="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('get-involved.donate') }}"
                   class="inline-flex items-center justify-center px-8 py-4 bg-brand-gold text-brand-purple rounded-xl font-bold hover:bg-brand-gold-light transition-colors shadow-lg">
                    Donate Now
                </a>
    ...
    <section class="py-20 bg-brand-cream">
        <div class="max-w-5xl mx-auto px-4 sm:px-6">
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold text-brand-purple">Our Mission &amp; Vision</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-white rounded-3xl p-8 border border-purple-100 shadow-sm">
                    <div class="w-12 h-12 rounded-xl bg-brand-purple flex items-center justify-center mb-5">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                        </svg>
                    </div>
                    <h3 class="text-xl font-bold text-brand-purple">Our Mission</h3>
                ...
                <div class="bg-brand-purple rounded-3xl p-8 shadow-sm">
    ...
    <section class="py-20 bg-white">
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            <div class="text-center mb-12">
                ...
                <h2 class="text-3xl sm:text-4xl font-bold text-brand-purple">The Five Pillars</h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <a href="{{ route('get-involved.donate') }}"
                   class="group block bg-brand-gold/10 rounded-2xl p-6 border-2 border-dashed border-brand-gold/40 hover:border-brand-gold hover:bg-brand-gold/20 transition-all text-center flex flex-col items-center justify-center min-h-[200px]">
                    ...
                    <h3 class="text-brand-purple font-bold text-lg">Support a Pillar</h3>
                    <p class="mt-2 text-gray-500 text-sm">Your donation directly funds one of these five life-changing programs.</p>
                    <p class="mt-4 text-brand-purple text-sm font-semibold group-hover:text-brand-gold transition-colors">Give Now →</p>
                </a>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6">
            <div class="flex items-end justify-between mb-10">
                <div>
                    ...
                    <h2 class="text-2xl sm:text-3xl font-bold text-brand-purple">Latest Updates</h2>
                </div>
                <a href="{{ route('impact') }}" class="text-brand-purple text-sm font-semibold hover:text-brand-gold transition-colors hidden sm:block">View all →</a>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach([
                    ['Foundation Launch', 'The Deborah Bonat Foundation is officially launched in loving memory of Mama Deborah Ishaku Bonat.', 'Foundation News'],
                    ...
                    <article class="bg-white rounded-2xl overflow-hidden border border-purple-100 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div class="p-6">
                            <span class="inline-block px-3 py-1 bg-brand-purple/10 text-brand-purple text-xs font-semibold rounded-full">{{ $category }}</span>
                            <h3 class="mt-3 font-bold text-gray-800">{{ $title }}</h3>
                            <p class="mt-2 text-gray-500 text-sm leading-relaxed">{{ $excerpt }}</p>
                            <a href="{{ route('impact') }}" class="mt-4 inline-block text-brand-purple text-sm font-semibold hover:text-brand-gold transition-colors">Read more →</a>
                        </div>
                    </article>
                @endforeach
            </div>
