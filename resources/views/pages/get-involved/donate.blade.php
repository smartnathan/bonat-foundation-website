    ...
    <section class="py-20 bg-brand-cream">
        <div class="max-w-5xl mx-auto px-4 sm:px-6">
            ...
            <div class="text-center mb-12">
                ...
                <h2 class="text-3xl font-bold text-brand-purple">Choose Your Giving Type</h2>
            </div>
            ...
            <div x-data="{ giving: 'one-time' }" class="bg-white rounded-3xl border border-purple-100 shadow-sm overflow-hidden mb-12">
                ...
                <div class="flex border-b border-gray-100">
                    <button
                        ...
                        :class="giving === 'one-time' ? 'bg-brand-purple text-white' : 'text-gray-500 hover:text-brand-purple'"
                    >
                        One-Time Gift
                    </button>
                    <button
                        @click="giving = 'legacy'"
                        class="flex-1 py-4 text-sm font-semibold transition-colors"
                        :class="giving === 'legacy' ? 'bg-brand-purple text-white' : 'text-gray-500 hover:text-brand-purple'"
                    >
                ...
                <div class="p-8 sm:p-12">
                    <div x-show="giving === 'one-time'">
                        <h3 class="text-xl font-bold text-brand-purple mb-2">Make a One-Time Gift</h3>
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <select class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30 focus:border-brand-purple">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-3">
                                    @foreach(['2,500', '5,000', '10,000', '25,000', '50,000'] as $amount)
                                        <button class="py-2.5 text-sm font-semibold border border-gray-200 rounded-xl hover:border-brand-purple hover:text-brand-purple transition-colors">
                                            ₦{{ $amount }}
                                        </button>
                                    @endforeach
                                </div>
                                <input type="number" placeholder="Enter custom amount" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-purple/30">
                            </div>
                        </div>
                    </div>

                    <div x-show="giving === 'legacy'" style="display:none">
                        <h3 class="text-xl font-bold text-brand-purple mb-2">Set Up a Legacy Gift</h3>
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <button class="py-3 text-sm font-semibold border-2 border-brand-purple text-brand-purple rounded-xl">Monthly</button>
                                    <button class="py-3 text-sm font-semibold border border-gray-200 text-gray-500 rounded-xl hover:border-brand-purple hover:text-brand-purple transition-colors">Annual</button>
                                </div>
                            </div>
                            <div>
                    ...
                    <div class="mt-8 p-5 bg-brand-cream rounded-2xl border border-purple-100">
                        <p class="text-brand-purple font-semibold text-sm mb-1">Bank Transfer Details</p>
                        ...
                        <p class="text-gray-400 text-xs mt-3">Online payment gateway integration coming soon. Please use bank transfer in the meantime and <a href="{{ route('contact') }}" class="text-brand-purple underline">notify us</a> after payment.</p>
                    </div>

                    <button class="mt-6 w-full py-4 bg-brand-gold text-brand-purple rounded-xl font-bold hover:bg-brand-gold-light transition-colors text-sm">
                        Proceed to Payment
                    </button>
                </div>
            </div>
            ...
            <div class="text-center mb-8">
                <h3 class="text-2xl font-bold text-brand-purple">Give to a Specific Pillar</h3>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach([
                    ...
                    <div class="bg-white rounded-2xl p-6 border border-purple-100 shadow-sm text-center">
                        ...
                        <h4 class="font-bold text-brand-purple">{{ $name }}</h4>
                        <p class="text-brand-gold text-xs font-semibold mt-1">{{ $initiative }}</p>
                        <a href="?fund={{ $fund }}" class="mt-4 inline-block px-5 py-2 bg-brand-purple text-white rounded-lg text-sm font-semibold hover:bg-brand-purple-light transition-colors">
                            Give to {{ $name }}
                        </a>
                    </div>
                @endforeach
